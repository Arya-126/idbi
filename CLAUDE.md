...
- `backend/app/main.py` — FastAPI app: consent (`/api/consent`, revoke, `/api/consent/log`), data (`/api/msme/{gstin}/data-pack` and `/health-card`), portfolio (`/api/portfolio` + refresh), impact (`/api/impact`), ULI/OCEN sim (`/api/uli/pull`, `/api/ocen/loan-request`), applications (`/api/msme/{gstin}/apply`, `/api/applications`, `/sanction`). `startup` hook kicks off model training + portfolio build in a **background daemon thread** so the port binds immediately (Render's health check would fail if boot blocked ~10s on cold model train + book build). `get_model()` and `build_portfolio()` are lock-guarded double-checked singletons, so an early request that races the warmup pays the cost once instead of double-training.
